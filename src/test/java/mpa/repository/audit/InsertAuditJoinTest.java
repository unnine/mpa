...
import mpa.fixture.repository.audit.config.TestMemberAuditConfiguration;
...
public class InsertAuditJoinTest {
    ...
    void byEntity_createUser_successChangeValue() {
        ...
        transactionTestTemplate.run(new TransactionTestTemplate.Runner() {
            ...
            @Transactional
            public void then() {
                verify(auditTestListener, times(2)).beforeCommit(auditTrailsArgumentCaptor.capture());
                verify(auditTestListener, times(2)).afterCommit(auditTrailsArgumentCaptor.capture());
